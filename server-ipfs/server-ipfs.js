...
const IPFS = require("../server-ipfs/model/models");
...
app.get("/apis/ipfsFiles", async (req, res) => {
    const { walletAddress } = req.query;
    if (!walletAddress) {
...
app.post("/apis/deleteMultipleIpfsFiles", async (req, res) => {
...
app.post("/apis/uploadToIPFS", upload.single("file"), async (req, res) => {
    if (!req.file) {
